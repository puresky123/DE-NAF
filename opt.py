...
def config_parser(cmd=None):
    ...
    parser.add_argument('--config', is_config_file=True, default='config/chest.txt', help='config file path')
    parser.add_argument("--expname", type=str, help='experiment name')
    parser.add_argument("--basedir", type=str, default='./log', help='where to store ckpts and logs')
    parser.add_argument("--evaldir", type=str, default='./log/eval_results', help='where to store ckpts and logs')
    ...
    parser.add_argument("--num_projs", type=int, default=50, help='how many projs were used in the training')
    parser.add_argument("--get_seed", type=int, default=0, help='whether to use fixed random seed')

    parser.add_argument('--with_depth', action='store_true')
    parser.add_argument('--downsample_train', type=float, default=1.0)
    parser.add_argument('--downsample_test', type=float, default=1.0)

    parser.add_argument('--model_name', type=str, default='deNafVMSplit', choices=['deNafVMSplit', 'mlp'])
    ...
    parser.add_argument("--s3im_patch_height", type=int, default=32)
    parser.add_argument("--s3im_patch_width", type=int, default=32)

    # loader options
    parser.add_argument("--batch_size", type=int, default=4096)
    parser.add_argument("--netchunk", type=int, default=409600)
    parser.add_argument("--n_iters", type=int, default=30000)
    parser.add_argument("--epochs", type=int, default=1500)
    parser.add_argument('--dataset_name', type=str, default='tigre', choices=['blender', 'llff', 'nsvf', 'dtu','tankstemple', 'own_data', 'tigre'])
    ...
    parser.add_argument("--lr_decay_iters", type=int, default=-1, help = 'number of iterations the lr will decay to the target ratio; -1 will set it to n_iters')
    parser.add_argument("--lr_decay_target_ratio", type=float, default=0.1, help='the target decay ratio; after decay_iters inital lr decays to lr*ratio')
    ...
    # model
    # volume options
    parser.add_argument("--net_type", type=str, default="tensor", help='which network to use')
    parser.add_argument("--store_way", type=str, default="Hash_Tensor", help='which way to store')
    ...
    parser.add_argument("--data_dim_sigma", type=int, default=27)
    parser.add_argument("--tensor_level", type=int, default=1, help=' The level of tensor multi-scale ')

    parser.add_argument("--rm_weight_mask_thre", type=float, default=0.0001, help='mask points in ray marching')
    parser.add_argument("--alpha_mask_thre", type=float, default=0.0001, help='threshold for creating alpha mask volume')
    ...
    # network decoder
    parser.add_argument("--shadingMode", type=str, default="Hash_MLP", help='which shading mode to use')
    ...
    parser.add_argument("--featureC", type=int, default=32, help='hidden feature channel in MLP')
    # train or render or mesh
    ...
    parser.add_argument("--fea2denseAct", type=str, default='sigmoid')
    parser.add_argument('--ndc_ray', type=int, default=0)
    parser.add_argument('--nSamples', type=int, default=1e6, help='sample point each ray, pass 1e6 if automatic adjust')
    parser.add_argument('--n_samples', type=int, default=192, help='sample point each ray')
    ...
    parser.add_argument("--i_eval", type=int, default=250, help='Epoch for evaluation')
    parser.add_argument("--i_save", type=int, default=500, help='Epoch for saving')
